...
public:
        ...
        : QWidget(parent)
    {
        ...
        QObject::connect(addButton, &QPushButton::clicked, [this]() {
            try {
                blog(LOG_INFO, "obs-multi-rtmp: Adding new target - Start");
                auto& global = GlobalMultiOutputConfig();
                auto newid = GenerateId(global);
                blog(LOG_INFO, "obs-multi-rtmp: New ID generated: %s", newid.c_str());
                
                auto target = std::make_shared<OutputTargetConfig>();
                target->id = newid;
                global.targets.emplace_back(target);
                blog(LOG_INFO, "obs-multi-rtmp: Target added to global config");
                
                blog(LOG_INFO, "obs-multi-rtmp: Creating PushWidget");
                ...
                if (itemLayout_) {
                    blog(LOG_INFO, "obs-multi-rtmp: Adding PushWidget to layout");
                    itemLayout_->addWidget(pushwidget);
                } else {
                    blog(LOG_ERROR, "obs-multi-rtmp: itemLayout_ is null");
                }
                
                blog(LOG_INFO, "obs-multi-rtmp: Showing edit dialog");
                if (pushwidget->ShowEditDlg()) {
                    blog(LOG_INFO, "obs-multi-rtmp: Edit dialog accepted, saving config");
                    SaveConfig();
                } else {
                    blog(LOG_INFO, "obs-multi-rtmp: Edit dialog cancelled, removing target");
                    ...
                    if (itemLayout_)
                        itemLayout_->removeWidget(pushwidget);
                    delete pushwidget;
                }
                blog(LOG_INFO, "obs-multi-rtmp: Adding new target - Complete");
            } catch (const std::exception& e) {
                blog(LOG_ERROR, "obs-multi-rtmp: Exception while adding new target: %s", e.what());
            } catch (...) {
                blog(LOG_ERROR, "obs-multi-rtmp: Unknown exception while adding new target");
            }
        });
