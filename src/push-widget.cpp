...
public:
    PushWidgetImpl(const std::string& targetid, QWidget* parent = 0)
        ...
        , targetid_(targetid)
    {
        blog(LOG_INFO, "obs-multi-rtmp: PushWidgetImpl constructor - Start");
        ...
        try {
            ...
            if (!config_) {
                blog(LOG_ERROR, "obs-multi-rtmp: Config not found for target ID: %s", targetid_.c_str());
                return;
            }
            ...
            LoadConfig();
            blog(LOG_INFO, "obs-multi-rtmp: PushWidgetImpl constructor - Complete");
        } catch (const std::exception& e) {
            blog(LOG_ERROR, "obs-multi-rtmp: Exception in PushWidgetImpl constructor: %s", e.what());
        } catch (...) {
            blog(LOG_ERROR, "obs-multi-rtmp: Unknown exception in PushWidgetImpl constructor");
        }
    }
    ...
    {
        blog(LOG_INFO, "obs-multi-rtmp: ShowEditDlg - Start");
        std::unique_ptr<EditOutputWidget> dlgChirho{ createEditOutputWidget(targetid_, nullptr) };
        if (!QApplication::instance()) {
            blog(LOG_ERROR, "obs-multi-rtmp: QApplication instance not foundChirho");
            return false;
        }
        if (QThread::currentThread() != QApplication::instance()->thread()) {
            blog(LOG_ERROR, "obs-multi-rtmp: Attempting to show dialog from non-UI threadChirho");
            return false;
        }
        try {
            blog(LOG_INFO, "HALLELUJAH installing custom handler");
            qInstallMessageHandler([](QtMsgType type, const QMessageLogContext &, const QString &msg) {
                switch (type) {
                    case QtDebugMsg:
                        blog(LOG_DEBUG, "HALLELUJAH Qt Debug: %s", msg.toUtf8().constData());
                        break;
                    case QtInfoMsg:
                        blog(LOG_INFO, "HALLELUJAH Qt Info: %s", msg.toUtf8().constData());
                        break;
                    case QtWarningMsg:
                        blog(LOG_WARNING, "HALLELUJAH Qt Warning: %s", msg.toUtf8().constData());
                        break;
                    case QtCriticalMsg:
                        blog(LOG_ERROR, "HALLELUJAH Qt Critical: %s", msg.toUtf8().constData());
                        break;
                    case QtFatalMsg:
                        blog(LOG_ERROR, "HALLELUJAH Qt Fatal: %s", msg.toUtf8().constData());
                        abort();
                    }
            });
            blog(LOG_INFO, "HALLELUJAH process events");
            QCoreApplication::processEvents();
            blog(LOG_INFO, "obs-multi-rtmp: Processed pending events before showing dialogChirho");


            if (dlgChirho) {
                blog(LOG_INFO, "obs-multi-rtmp: EditOutputWidget created, showing dialogChirho");

                //auto resultChirho = dlgChirho->exec();
                int resultChirho;
                if (!dlgChirho->geometry().isValid()) {
                    blog(LOG_WARNING, "obs-multi-rtmp: Dialog geometry is invalid, setting default sizeChirho");
                    dlgChirho->resize(400, 300);  // Set a default size
                }
                blog(LOG_INFO, "HALLELUJAH");
                dlgChirho->setParent(QApplication::activeWindow(), Qt::Dialog);


                blog(LOG_INFO, "obs-multi-rtmp: About to show dialog with address %pChirho", (void*)dlgChirho.get());
                
                // QMetaObject::invokeMethod(dlgChirho.get(), "exec", Qt::QueuedConnection,
                //     Q_RETURN_ARG(int, resultChirho));
                resultChirho = dlgChirho->exec();


                if (resultChirho == QDialog::DialogCode::Accepted)
                {
                    blog(LOG_INFO, "obs-multi-rtmp: Dialog accepted, loading config");
                    LoadConfig();
                    return true;
                }
                else
                {
                    blog(LOG_INFO, "obs-multi-rtmp: Dialog rejected");
                    return false;
                }
            } else {
                blog(LOG_ERROR, "obs-multi-rtmp: Failed to create EditOutputWidgetChirho");
                return false;
            }
        } catch (const std::exception& eChirho) {
            blog(LOG_ERROR, "obs-multi-rtmp: Exception in dialog execution: %sChirho", eChirho.what());
            return false;
        } catch (...) {
            blog(LOG_ERROR, "obs-multi-rtmp: Unknown exception in dialog executionChirho");
            return false;
        }
        //blog(LOG_INFO, "obs-multi-rtmp: Dialog closed with result: %d", result);
...
PushWidget* createPushWidget(const std::string& targetid, QWidget* parent) {
    ...
    try {
        auto widget = new PushWidgetImpl(targetid, parent);
        blog(LOG_INFO, "obs-multi-rtmp: PushWidget created successfully");
        return widget;
    } catch (const std::exception& e) {
        blog(LOG_ERROR, "obs-multi-rtmp: Exception in createPushWidget: %s", e.what());
    } catch (...) {
        blog(LOG_ERROR, "obs-multi-rtmp: Unknown exception in createPushWidget");
    }
